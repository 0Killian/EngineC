...
typedef struct renderer_system_state renderer_system_state;
 ...
 */
b8 renderer_init(renderer_system_state *state, u64 *size_requirement, const window *window);
 ...
 */
void renderer_deinit(renderer_system_state *state);
 ...
 * @retval FALSE Failure
 */
b8 renderer_frame_prepare(renderer_system_state *state, frame_packet *packet);
 ...
 * @retval FALSE Failure
 */
b8 renderer_command_list_begin(renderer_system_state *state, frame_packet *packet);
 ...
 * @retval FALSE Failure
 */
b8 renderer_command_list_end(renderer_system_state *state, frame_packet *packet);
 ...
 * @param[in,out] packet A pointer to the frame packet to render.
 */
b8 renderer_frame_render(renderer_system_state *state, frame_packet *packet);
