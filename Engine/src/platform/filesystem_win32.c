...
b8 filesystem_node_write(const char *path, void *content, u64 size, b8 create) {
    HANDLE handle = CreateFileA(
        path, GENERIC_WRITE, FILE_SHARE_READ, NULL, create ? CREATE_ALWAYS : OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (handle == INVALID_HANDLE_VALUE) {
        return FALSE;
    }
 ...
 * @retval FALSE Failure
 */
b8 filesystem_handle_close(filesystem_handle handle) { return CloseHandle(handle); }
 ...
 * @retval FALSE Failure
 */
b8 filesystem_node_delete(const char *path) { return DeleteFileA(path); }
